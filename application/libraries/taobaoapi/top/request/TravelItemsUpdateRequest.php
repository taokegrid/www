 ...
 */
class TravelItemsUpdateRequest
{
	/** 
	...
	/** 
	 * 商品库存。如果发布旅游度假线路宝贝，该字段可以忽略，参考extend字段。
	 **/
	private $num;
	...
	/** 
	 * 商品一口价。如果发布旅游度假线路宝贝，该字段可以忽略，参考extend字段。
	 **/
	private $price;
	...
	/** 
	 * Sku销售属性串对应的价格，每一个属性串都会对应一个价格，精确到两位小数，单位为元。sku_prices的数组大小应该和sku_properties的数组大小一致。如果发布线路的商品，请参考extend字段。
	 **/
	private $skuPrices;
	
	/** 
	 * Sku销售属性串，调用taobao.travel.itemsprops.get接口获取商品销售属性code，然后拼接成pid:vid;pid:vid格式。如果发布线路的商品，请参考extend字段。
	 **/
	private $skuProperties;
	
	/** 
	 * Sku销售属性串对应的库存，每一个属性串都会对应一个库存，显然sku_quantities的数组大小应该和sku_properties的数组大小一致。如果发布线路的商品，请参考extend字段。
	 **/
	...
	public function check()
	{
		...
		RequestCheckUtil::checkMaxListSize($this->skuPrices,20,"skuPrices");
		RequestCheckUtil::checkMaxListSize($this->skuProperties,20,"skuProperties");
		RequestCheckUtil::checkMaxListSize($this->skuQuantities,20,"skuQuantities");
	}
